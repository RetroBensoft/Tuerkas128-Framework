...
* Perseus uses screen areas, breath areas and portals. These features are strongly integrated in Tukerkas128 Framework because of design decisions (the main goal of Tuerkas128 Framework is produce highly customizable games, so maybe those were bad decisions in hindsight). But if a game developed with Tuerkas128 Framework does not need some of these features, it wouldn't be difficult to get rid of them by modifying core routines. Basicly, deleting all routines in `framework\tuerkas128_code_entity_SA.asm`, `framework\tuerkas128_code_entity_BA.asm` and `framework\tuerkas128_code_entity_PRTL.asm` , and then removing carefully from the code all calls to those routines.
* The way Tukeras128 Framework detects collisions with screen items and objetcs is sort of peculiar, but it is suitable for Perseus and for 99% of videogames. For the remaining 1%, some changes would have to be made in `framework\tuerkas128_code_block_col.asm`
